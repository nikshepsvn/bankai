...
parameter language model. Through fifteen experiments, we demonstrate that:
...
\textbf{generalize to held-out prompts}---originally reported as fixing
4~of~17 problems (23.5\%), a figure since corrected. Measured soundly that
patch is net negative; the corrected search (Experiment~12) fixes
\textbf{5 of the 16 held-out problems the base model gets wrong (31\%) with
zero breakage}, verified in free generation.
...
\subsection{Extended Layer Set: A Negative Result}

Experiment~6 speculated that trigonometry and exponential derivatives saw zero
fixes because layers 5, 6 and 10---identified as high-impact for calculus---were
absent from the search set. Rerunning with $[1,2,3,4,5,6,10,34]$ and changing
nothing else falsifies this.

\begin{table}[h]
\centering
\caption{Baseline versus extended layer set, corrected metric, 300 iterations.}
\begin{tabular}{lcc}
\toprule
 & Baseline $[1,2,3,4,34]$ & Extended $+[5,6,10]$ \\
\midrule
Train (generation) & 30/60 $\to$ 33/60 & 30/60 $\to$ \textbf{35/60} \\
Held-out (generation) & 14/30 $\to$ \textbf{19/30} & 14/30 $\to$ 17/30 \\
Held-out fixed & \textbf{5} & 3 \\
Held-out broke & 0 & 0 \\
...
The extended search fits the training set better and generalizes worse. Because
training accuracy \emph{improved}, this is overfitting rather than under-sampling
of the larger candidate pool. The hypothesis fails on its own terms: trigonometry
is unmoved (2/5 to 2/5) and second derivatives are unmoved (0/5 to 0/5).

This replicates the Experiment~10 pattern from an independent direction. There,
adding attention projections eliminated all held-out fixes; here, adding MLP
layers halves them. Two unrelated expansions of the search space at fixed
iteration budget both traded generalization for training fit, suggesting the
constraint is a property of greedy hill climbing on this objective rather than of
any particular layer or projection type. The narrower search set is the better
one, and Experiment~6's layer choice was sound even though its stated explanation
for the trig gap was not.

Both runs used 300 iterations while the candidate pool grew from ${\sim}131$K to
${\sim}210$K rows, so the extended run sampled a smaller fraction of its space. A
coverage-matched rerun would separate ``more layers hurt'' from ``more layers need
...
% ============================================================
\section{Experiment 14: Head-to-Head Patch Comparison}
\label{sec:exp14}

Experiment~6 reported 4 held-out fixes and Experiment~12 reported 5, under
different probe sets and different metrics, so the two are not directly
comparable. Experiment~14 scores every patch on the same 30 held-out probes with
the same corrected metric. Evaluation only; no search.

\begin{table}[h]
\centering
\caption{All patches on one yardstick: 30 held-out probes, greedy generation.}
\begin{tabular}{lccccc}
\toprule
Patch & Flips & Bytes & Held-out & Fixed & Broke \\
\midrule
(no patch) & --- & --- & 14/30 & --- & --- \\
Experiment~6 (\texttt{token\_gap}) & 93 & 1{,}116 & \textbf{13/30} & 2 & \textbf{3} \\
\textbf{Experiment~12 (corrected)} & 78 & 936 & \textbf{19/30} & \textbf{5} & \textbf{0} \\
Experiment~12 (ext.\ layers) & 95 & 1{,}140 & 17/30 & 3 & 0 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Finding.} On a level playing field the Experiment~6 patch is net
negative: it fixes 2 held-out probes and breaks 3, leaving the model worse than
unpatched. Experiment~6's claim of zero breakage was itself a measurement
artifact---the single-token metric was too coarse to detect regressions that
appear once the model must produce a complete expression. Per-category, trigonometry
falls from 2/5 to 1/5 and exponential derivatives from 4/5 to 3/5 under that patch,
while the Experiment~12 patch holds or improves every category.

In fairness, the Experiment~6 patch was optimized against a different objective,
so underperforming here is expected. But the claim attached to it---fixing 4 of
17 problems the base model gets wrong---was a claim about capability rather than
about logit gaps, and on those terms it does not survive sound measurement. The
correction moves the method from $-1$ net to $+5$ net on the same yardstick,
using 15 fewer flips and 180 fewer bytes.

% ============================================================
\section{Experiment 15: Calculus Layer Impact Map}
\label{sec:exp15}

Experiment~6 explained its absent trigonometry and exponential-derivative fixes
by asserting that a calculus-specific layer impact map identified layers 5, 6 and
10 as high-impact for calculus but absent from the search set. No artifact for
that map was ever committed. Experiment~15 measures it, following
Experiment~2A's method with calculus probes and the corrected metric: XOR the
entire MLP of one layer, measure the mean absolute change in \texttt{seq\_logprob}
across all 90 calculus probes, restore, repeat for all 36 layers.

\begin{table}[h]
\centering
\caption{Top 12 of 36 layers by mean $|\Delta|$ across 90 calculus probes.}
\begin{tabular}{clcl}
\toprule
Rank & Layer & Mean $|\Delta|$ & Status \\
\midrule
...
6 & 0 & 7.31 & --- \\
7 & 9 & 5.74 & --- \\
8 & \textbf{5} & 5.15 & \textbf{claimed high-impact} \\
9 & \textbf{10} & 4.74 & \textbf{claimed high-impact} \\
10 & 8 & 4.58 & --- \\
11 & 11 & 4.55 & --- \\
12 & \textbf{6} & 4.46 & \textbf{claimed high-impact} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Finding.} The premise was false. The five layers Experiments~3--6
searched, $[1,2,3,4,34]$, are ranks 1 through 5 for calculus. The three layers
claimed to be high-impact rank 8th, 9th and 12th, below even layers 0 and 9,
which nobody proposed adding.

This explains the extended-layer result of Section~\ref{sec:exp12} mechanically
rather than statistically. Adding layers 5, 6 and 10 did not extend the search
into more promising territory; it diluted an already optimal candidate pool,
spending a fixed iteration budget on rows with roughly half the behavioral
leverage. Better training fit with worse generalization is what that predicts.

Experiment~6's account of the trigonometry gap was therefore wrong twice: the
hypothesis is falsified in Section~\ref{sec:exp12}, and the premise it rested on
is contradicted here. The original layer selection was not a limitation but the
best available choice, and the trigonometry gap requires a different explanation.
If the search set is widened in future work, this map points to layers 0 and 9.
...
changing nothing else, gives 2 held-out fixes rather than 4. Scored on the
corrected probe set (Section~\ref{sec:exp14}) that patch is in fact net negative:
2 fixed against 3 broken, so even the zero-breakage claim was an artifact of the
coarse metric.
...
\textbf{The corrected metric is MLX-only.} \texttt{seq\_logprob} requires
\texttt{Backend.seq\_logprobs()}, implemented for MLX but not for GGUF: the
\texttt{bankai\_eval} subprocess exposes a single-gap \texttt{PROBE} command and
would need a continuation-scoring command alongside it. Until then,
Experiments~9--11 rest on \texttt{token\_gap} and inherit its defects; their
numbers warrant the same caution as Experiment~6's. The GGUF backend raises
\texttt{NotImplementedError} rather than silently falling back.
...
Through fifteen experiments on Bonsai~8B, we characterized the method's
