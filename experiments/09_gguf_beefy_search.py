"""
Experiment 6 beefier: 800 iterations, expanded layer set.

Adds layers 5, 6, 10 (identified as high-impact for calculus in the
earlier layer analysis but not in the original 5-layer search set).
"""

import modal

app = modal.App("bankai-exp6-beefier")
...
image = (
    ...
    # Stable layer: llama.cpp + common built once, cached
    .run_commands(
    ...
    # Dynamic layer: bankai_eval (rebuilds on source edits)
...
def run_beefier_exp6():
    ...
    print(f"[beefy] Training: {len(ALL_TRAIN)} probes, Validation: {len(ALL_VAL)} probes")
    ...
    print(f"[beefy] Loaded in {time.time()-t0:.1f}s")

    # Expanded layer set: add calculus-impactful layers 5, 6, 10
    ...
    patch = greedy_search(
        ...
        max_iters=800,  # 2.7x more iterations
        control_penalty=2.0,
        fitness_mode="mean",
        seed=42,
        patch_name="calculus_beefy_v1",
        patch_description="Experiment 6 beefier: 800 iters, 8 layers, L40S GGUF",
        base_model="prism-ml/Bonsai-8B-gguf",
    )
    search_time = time.time() - t0
    print(f"\n[beefy] Search complete in {search_time:.1f}s")
    print(f"[beefy] Accepted: {len(patch.flips)}")

    # Measure training and validation sets
    # Restart subprocess to get clean baseline
    backend._start_process()  # model reloaded = unpatched
...
def main():
    result = run_beefier_exp6.remote()
    print("\n" + "=" * 60)
    print("BEEFIER EXP 6 RESULTS")
    print("=" * 60)
    ...
    import json
    with open("patches/calculus_beefy_v1.json", "w") as f:
        json.dump(result["patch"], f, indent=2)
    print("\nPatch saved to patches/calculus_beefy_v1.json")
